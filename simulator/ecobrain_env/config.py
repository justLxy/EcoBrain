...
TIERS = {
    ...
    "high": {
        "target_inventory": 5,        # 理想库存 (非常稀缺)
        "current_inventory": 2,       # 训练开始时的实际库存
        "price_min": 50000.0,         # 判定为高价值的最低底价 (和 Java 端的 config.yml 对应)
        "price_max": float('inf'),    # 最高价无上限
        "price_target": 500000.0,     # AI 训练时，把价格拉升到多少以上会获得顶级奖励
    ...
    # 中等价值物品 (如: 副本材料, 铁锭, 金锭)
    "mid": {
        "target_inventory": 50,       # 理想库存
        "current_inventory": 25,      # 训练开始时的实际库存
        "price_min": 1000.0,          # 判定为中等价值的最低底价
        "price_max": 50000.0,         # 判定为中等价值的最高底价
    ...
    # 低价值物品 (如: 泥土, 小麦, 圆石)
    "low": {
        "target_inventory": 1000,     # 理想库存 (极其充裕)
        "current_inventory": 500,     # 训练开始时的实际库存
        "price_min": 0.0,             # 最低价
        "price_max": 1000.0,          # 判断为低价值的最高底价
        "price_target": 50.0,         # AI 训练时，必须把价格死死压制在这个值以下
