...
本文档按论文结构解释该系统的理论动机、模块组织、数据流、数学接口与验证方式，并特别讨论为何要采用零信任 IPO、虚拟库存与真实库存分离、单模型连续观测、交易日志半回放训练，以及插件化模块分层。
...
\[
s_t=\min\left(0.999,\;0.05 + \frac{1}{2}\frac{|P_t-\mathrm{TWAP}_t|}{\max(\varepsilon,\mathrm{TWAP}_t)} + \tau_t^{\text{dump}}\right).
\]
...
| 维度 | 公式 | 设计动机 |
| --- | --- | --- |
| `saturation` | \(\frac{I_t}{\max(1,I_t^\star)}\) | 衡量曲线库存紧张度；它直接决定价格相对目标库存的偏离。 |
| `log_target_inventory` | \(\operatorname{clip}(\log(1+I_t^\star),0,20)\) | 让模型区分“小宗高频物品”和“大宗材料”，避免一个模型只学到单一库存尺度。 |
| `log_physical_stock` | \(\operatorname{clip}(\log(1+S_t),0,20)\) | 告诉模型系统真实有多少货，帮助它分辨“曲线缺货”和“仓库缺货”。 |
| `k_factor` | \(\operatorname{clip}(k_t,k_{\min},k_{\max})\) | 显式暴露当前曲线陡峭度，否则策略无法知道自己站在什么曲线上。 |
| `physical_ratio` | \(\operatorname{clip}\left(\frac{S_t}{\max(1,I_t^\star)},0,1000\right)\) | 与 `saturation` 配合，揭示虚拟库存和真实库存是否发生结构性分离。 |

#### 5.1.2 Flow and Macro Features

| 维度 | 公式 | 设计动机 |
| --- | --- | --- |
| `recent_flow` | \(\frac{\sum \text{BUY qty}-\sum \text{SELL qty}}{\text{schedule minutes}}\) | 用净流速而不是成交额，刻画当前买压或卖压方向。 |
| `global_inflation` | \(\frac{\sum \text{SELL money}-\sum \text{BUY money}}{\max(\varepsilon,\mathrm{AOV}_t)}\) | 近似系统净印钞强度；用 \(\mathrm{AOV}_t\) 归一化是为了跨服务器稳定量级。 |
| `has_activity_trade` | \(\mathbb{1}[\text{activity volume}>0]\) | 这是动作门控信号；无活动时系统只允许小幅修正。 |
| `log_activity` | \(\operatorname{clip}\left(\log\left(1+\frac{\text{activity volume}}{\text{schedule minutes}}\right),0,20\right)\) | 区分“完全无交易”和“有交易但很冷清”。 |
| `log_treasury` | \(\operatorname{clip}\left(\log\left(1+\frac{T_t}{\max(\varepsilon,\mathrm{AOV}_t)}\right),0,20\right)\) | 让金库规模在不同服务器经济体量下可比较。 |

#### 5.1.3 Price Anchor Features

| 维度 | 公式 | 设计动机 |
| --- | --- | --- |
| `volatility` | \(\frac{|P_t-\mathrm{TWAP}_t|}{\max(\varepsilon,\mathrm{TWAP}_t)}\) | 衡量当前价偏离真实成交锚点的程度，也与 SELL 税耦合。 |
| `log_price` | \(\operatorname{clip}(\log(\max(\varepsilon,P_t)),-20,20)\) | 价格跨度大时，对数尺度比原始价格更稳定。 |
| `log_twap` | \(\operatorname{clip}(\log(\max(\varepsilon,\mathrm{TWAP}_t)),-20,20)\) | 提供较慢的市场公允价锚点。 |
| `price_vs_twap` | \(\operatorname{clip}\left(\log\frac{\max(\varepsilon,P_t)}{\max(\varepsilon,\mathrm{TWAP}_t)},-10,10\right)\) | 不只告诉模型“价格多大”，还告诉模型“现在是偏贵还是偏便宜”。 |
| `price_change_pct` | \(\operatorname{clip}\left(\frac{u_t-u_{t-1}}{\max(\varepsilon,u_{t-1})},-10,10\right)\) | 这里的 \(u_t\) 是周期成交均价；它更接近玩家真实感知，而不是 AI 内部参数变化。 |
| `log_base_price` | \(\operatorname{clip}(\log(\max(\varepsilon,b_t)),-20,20)\) | 让策略知道自己正在控制的底价基线。 |
| `elasticity` | \(\operatorname{clip}\left(\frac{\text{recent\_flow}}{\max(\varepsilon,100|\text{price\_change\_pct}|)},-10^4,10^4\right)\) | 这是启发式弹性特征，描述需求流速对价格扰动的敏感程度。 |

#### 5.1.4 Temporal Feature

| 维度 | 公式 | 设计动机 |
| --- | --- | --- |
| `log_age` | \(\operatorname{clip}(\log(1+\text{listing age in cycles}),0,20)\) | 新物品和成熟物品面对同样交易信号时，最优调节方向可能不同，因此年龄必须显式编码。 |
...
按当前代码树直接执行测试，Java 侧 `mvn test` 结果为 \(19/19\) 通过；Python 侧 `python3 -m pytest simulator/tests -q` 结果为 \(2/3\) 通过、\(1/3\) 失败。也就是说，仓库已经具备一组比“示例跑通”更强的验证，但它们并非全部处于无争议通过状态。

#### 8.1.1 Java-Side Analytical Tests
Java 侧全部通过的测试支持以下命题。

- `AMMCalculatorTest` 验证当前价公式、买卖离散求和和基础 spread 数值是否符合理论表达。
- `PricingSimulationTest` 验证“批量买入成本等于逐笔买入成本之和”“买后立刻卖会因 spread 亏损”“当 \(k\) 更大且库存更低时，买价更高”等定性规律。
- `EdgeCaseSimulationTest` 验证极端库存和非法数量下的边界行为，防止曲线跌入未定义区域。
- `ItemMarketRepositoryTargetScalingTest` 验证更新 \(I_t^\star\) 时，比例缩放 \(I_t\) 不会污染 \(S_t\)。
- `ItemOperationCoordinatorTest` 验证同一 `item_hash` 上的操作确实串行，从而支撑交易与 AI 调参并发安全。
- `OnnxModelRunnerTest` 验证没有模型文件时系统返回中性动作 \((1.0,0.0)\)，而不是崩溃。
- `BulkSellListenerTest` 与 `LeaderboardClearTest` 验证交互与榜单的基本正确性。

#### 8.1.2 Simulator Contract Tests
Python 侧当前通过的测试支持以下命题。

- `test_amm_contract.py` 验证动态 spread 中的波动项确实与 \(|P_t-\mathrm{TWAP}_t|/\mathrm{TWAP}_t\) 一致。
- `test_env_contract.py` 中的数据回放测试验证了 `low` 世界优先抽取低价数据桶，确保训练 bucket 选择逻辑没有漂移。

但同一文件中的 `test_no_activity_gate_holds_policy_action` 当前失败。失败原因并不是测试环境损坏，而是模拟器实现与测试假设已经出现契约漂移：测试假设“无活动时完全 HOLD”，而当前 `env.py` 的实际实现是“无活动时按 \(\gamma=0.35\) 衰减动作而非归零”。在实测中，满幅正动作会把 `base_price` 从 \(100.0\) 推到约 \(104.2\)，这与 README 所描述的“弱调控而非绝对静止”一致。
...
需要明确的是，仓库当前没有附带固定 benchmark 数字，因此本文档不把任何单次训练结果写成“最终结论”。换句话说，EcoBrain 已经给出了实验协议，但没有伪装成已经完成了跨服务器普适定标。

### 8.3 Threats to Validity
现有验证仍有边界，主要体现在三个方面。

- 当前测试更擅长验证公式、合约和并发，而不是完整 Bukkit 生态下的端到端运行。
- ONNX 推理在“有真实模型文件时”的行为主要依赖接口一致性和外部训练产物，仓库内对这一路径的自动化覆盖仍有限。
- 模拟器当前存在一个可观测的测试契约漂移：README 与 `env.py` 都采用“无活动时衰减动作”的语义，但部分测试仍假设“无活动时完全 HOLD”。
- `PluginSettings` 的代码默认值与仓库附带 `config.yml` 的 shipped 值并不完全一致，因此研究和部署时应以当前 `config.yml` 为主，而不是假设删除配置项后的回退值也代表推荐设置。

这些边界并不说明设计有问题，但它们提醒读者：EcoBrain 的强项是“机制设计清晰、验证点明确”，不是“已经对所有服务器环境做完穷尽性实验”。
