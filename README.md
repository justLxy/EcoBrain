...
*   **Base Price Multiplier (`[-20% ~ +20%]`)**：AI 直接决定基准价上涨或下跌的精准百分比。这里设定 20% 为单次调控的物理极限，既给予了 AI 足够的调整空间来应对市场变化，又防止了单次调价过猛导致物价瞬间崩溃（哪怕 AI “发疯”，单次最多也只能涨跌 20%）。**注意：这只是 AI 输出的范围，最终在 Java 端还会受到 `config.yml` 中 `per-cycle-max-change-percent` 的二次安全拦截**。
*   **K-Factor Delta (`[-0.1 ~ +0.1]`)**：AI 微调 AMM 曲线的陡峭程度。K 系数非常敏感（通常在 0.5 到 2.0 之间），一次性变动 0.1 已经足以明显改变滑点的深度。限制在 0.1 可以让曲线的形态变化像平滑的渐变，而不是剧烈的跳变。
