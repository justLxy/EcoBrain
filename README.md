...
```mermaid
flowchart TD
  A[reset: 初始化商品状态<br/>target/current/physical + base_price=0.01] --> B[AI action: 调 base_price 与 k]
  B --> C[Micro-ticks: 模拟玩家多轮买卖]
  C --> D[AMM: 滑点 + SELL 动态印花税<br/>(TWAP/volatility/dumpingTax)]
  D --> E[更新库存: physical_stock + current_inventory]
  E --> F[提取观测 obs(6维) + 计算 reward]
  F --> B
```
