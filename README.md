...
#### 3.0.4 线上“路由 + AI 调参”的运行时流程（按一个周期）
...
2) **构造 obs**：`[saturation, flow, inflation, elasticity, volatility, log_price]`。其中 `log_price = log(P_current)`，`P_current` 来自 AMM 公式而不是 `base_price`。  
3) **活跃门槛**：若该物品在活动窗口内成交额为 0，则本周期 **不跑 AI**（避免无人问津的物品被乱调）。  
4) **路由**：按 `base_price` 与 `target_inventory` 阈值分桶为 `high/mid/low`，选择对应 `ecobrain_{tier}_value.onnx` 推理。  
5) **推理**：ONNX 输出 2 维动作分量，插件端 clip 到 \([-1,1]\)，映射为 `basePriceMultiplier` 与 `kDelta`。  
6) **落地**：`base_price *= basePriceMultiplier`，`k += kDelta`，仅保留硬 clamp（tier-aware `base_price` 上限、`k` 上下限、全局最小底价等），然后写回数据库并记录审计事件。  
...
- **动作裁剪**：插件端会对 ONNX 输出做 `clip(out, -1, 1)` 作为安全裁剪（见 `OnnxModelRunner`），然后按 `ACTION_BASE_PRICE_MAX_PERCENT` 与 tier 的 `kDelta` 上限映射为真实调参幅度。
