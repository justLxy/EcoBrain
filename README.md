...
#### 扩展阅读：强化学习(RL)与深度学习(DL)的化学反应
很多开发者好奇：PPO 叫“强化学习”，但为什么里面全是“神经网络”和“权重”？
其实，这是两者缝合的产物，全称是**深度强化学习 (Deep Reinforcement Learning)**：
*   **前向传播 (DL 的招式)**：AI 接收到那 6 维经济数据，通过神经网络的复杂计算，吐出一个具体的动作（比如涨价 15%）。
*   **打分 (RL 的招式)**：把这个动作扔进模拟器执行，看虚拟玩家的反应，最后计算出一个 Reward 分数。
*   **反向传播 (DL 的招式)**：算法拿着这个分数，通过梯度下降/上升微调神经网络里的数万个参数（权重），让它下次更倾向于（或避免）做出同样的决策。
*   **在线推理 (Inference)**：当模型导出为 `.onnx` 放入 Java 插件后，AI 就**停止了学习**。它在 Minecraft 里只进行极低性能消耗的“纯前向传播计算”，以此彻底杜绝边跑边练带来的服务器卡顿和试错风险。
