# EcoBrain 3.0
## Abstract
EcoBrain 是一个面向 Minecraft 服务器的自适应系统市场插件。玩家可以把任意物品卖给系统市场获得金钱；一旦物品被卖入系统，系统就会为该物品建立市场记录与库存，随后其他玩家便可以直接从系统市场买走该物品。换句话说，EcoBrain 做的不是一张预先写死价格表的商店，而是一个会随着真实供给、真实需求和真实库存持续演化的全服市场。
...
使系统在真实交易事件 $e_t$ 驱动下，兼顾交易活跃性、库存稳定性、资金守恒与价格可解释性。与常见“动态商店”相比，EcoBrain 的设计重点不在于“让价格会动”，而在于让玩家能够自由地把物品卖入市场、让市场为这些物品形成可购买库存，并让所有价格变化都由真实库存、真实成交和真实资金能力共同约束。

本文档按论文结构解释该系统的理论动机、模块组织、数据流、数学接口与验证方式，并特别讨论为何要采用零信任 IPO、虚拟库存与真实库存分离、单模型连续观测、交易日志半回放训练，以及插件化模块分层。

## 1. Introduction
EcoBrain 在服务器里的直接作用很具体：它提供一个会自动调价的系统市场，允许玩家把任意物品卖给系统换取金钱；系统接收这些物品后，会把它们纳入市场库存，供其他玩家继续购买。也就是说，EcoBrain 试图把“玩家卖给系统”和“玩家从系统买走”连接成一个完整的流通回路，并让价格随着真实买卖行为自动变化，而不是要求管理员先为每一种物品手工编写价格。正因为它要同时处理自由上架、库存流转、玩家交易和自动定价，Minecraft 服务器经济系统在这里会暴露出三个长期问题。第一，价格往往由管理员预设，因此“动态”只是对静态价格表做局部扰动，而不是对稀缺性做真实响应。第二，很多系统没有严格的库存或资金约束，导致价格信号被无限供给或无限印钞污染。第三，训练环境与线上环境经常不是同一个问题，模型在离线看见的状态与线上接收的状态不一致，最终出现“模型没坏，但部署题目变了”的情况。
